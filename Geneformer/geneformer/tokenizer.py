    ...
    if file_format == "loom":
        ...
        with lp.connect(data_directory) as data:
            ...
            # Genes need to be collapsed
                ...
                mapped_vals = [gene_mapping_dict.get(gene.upper()) for gene in data.ra["ensembl_id"]]
                data.ra["ensembl_id_collapsed"] = mapped_vals
                dup_genes = [idx for idx, count in Counter(data.ra["ensembl_id_collapsed"]).items() if count > 1]
                ...
                for _, _, view in tqdm(data.scan(axis=1, batch_size=chunk_size), total=num_chunks):
                    ...
                    def process_chunk(view, duplic_genes):
                        ...
                        dup_data_df = data_count_view.loc[
                            data_count_view.index.isin([i for i in duplic_genes if "None" not in i])
                        ]
...
class TranscriptomeTokenizer:
    def __init__(
        ...
        **Parameters:**
        ...
        if not self.special_token:
            if ("<cls>" in self.gene_token_dict.keys()) and ("<eos>" in self.gene_token_dict.keys()):
                logger.warning(
    ...
    def tokenize_loom(self, loom_file_path, target_sum=10_000):
        ...
        with lp.connect(str(loom_file_path)) as data:
            ...
            coding_miRNA_loc = np.where([self.genelist_dict.get(i, False) for i in data.ra["ensembl_id_collapsed"]])[0]
            norm_factor_vector = np.array(
                [self.gene_median_dict[i] for i in data.ra["ensembl_id_collapsed"][coding_miRNA_loc]]
            )
            ...
            tokenized_cells = []
            for _ix, _selection, view in data.scan(items=filter_pass_loc, axis=1, batch_size=self.chunk_size):
