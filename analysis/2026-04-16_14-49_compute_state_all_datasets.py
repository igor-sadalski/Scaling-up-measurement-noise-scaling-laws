...
Step scaling: max_steps is computed automatically by the iterator as
max_epochs * batches_per_epoch, where max_epochs = max(1, 10 * max_size / size).
This gives a roughly constant total step budget across sizes (~10 * max_size / 64).
Early stopping is disabled; training runs for the full step budget.

Order: PBMC, larry, merfish (2 jobs/GPU), then shendure (1 job/GPU).
"""
...
for ds_name, cfg in DATASETS.items():
    ...
    print(f"  Using fixed step budget (no early stopping)")
    print(f"{'='*70}\n")
