"""Fine-tune the pretrained gc104M Geneformer on the PBMC noise-scaling grid.

Continues MLM (masked language modeling) pretraining of ctheodoris/Geneformer
(gc104M, 316M params, 1152H/18L/18-head) on every cell of the 10 sizes x 10
qualities PBMC sweep. For each (size, quality):

  1. Re-tokenize the train h5ad and the per-quality test h5ad with the official
     gc104M token dictionary (idempotent -- skip if already done).
  2. MLM continued-pretrain the pretrained checkpoint for EPOCHS epochs.
...
Fine-tuning here uses the same API as the canonical pretraining example at
`Geneformer/examples/pretraining_new_model/pretrain_geneformer_w_deepspeed.py`:
a declarative `training_args` dict, `BertForMaskedLM.from_pretrained`, and
`GeneformerPretrainer` driving the loop. We deliberately keep MLM (rather than
switching to `geneformer.Classifier`) because the MI estimand needs unbiased
cell embeddings, not a head trained on `celltype.l3`.

Outputs go to OUTPUT_BASE/geneformer_noise_scaling/, mirroring other/hp_tunning/.
Existing PBMC/Geneformer results in DATA_DIR/PBMC/.../results/Geneformer/ are
not touched.
...
OUTPUT_DIR = OUTPUT_BASE / "geneformer_noise_scaling"
...
def _fine_tune(train_ds_path: Path, model_out: Path, log_dir: Path,
    ...
    """Continue MLM pretraining the gc104M Geneformer on train_ds_path.

    Mirrors the call shape in
    Geneformer/examples/pretraining_new_model/pretrain_geneformer_w_deepspeed.py:
    declarative training_args dict, BertForMaskedLM + GeneformerPretrainer,
...
def run_one(size: int, quality: float, seed: int, device: int) -> dict:
    ...
    try:
        ...
        _fine_tune(train_tok / "tokenized.dataset", model_out, log_dir, EPOCHS, seed)
