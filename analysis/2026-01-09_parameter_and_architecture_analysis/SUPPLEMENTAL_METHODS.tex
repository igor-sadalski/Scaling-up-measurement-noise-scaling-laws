...
We use a reduced-parameter Geneformer architecture compared to many single-cell transformer models (Table~\ref{tab:hyperparameter_comparison}) to support training thousands of models across different dataset sizes, qualities, and random seeds—conditions where computational cost and overfitting are serious concerns. Larger models such as Geneformer-V1\cite{theodoris2023geneformer} (6 layers, 2048 tokens, pretrained on $\sim$30M cells), Geneformer-V2-104M\cite{chen2024geneformer} (12--18 layers, 4096 tokens, pretrained on $\sim$95M cells), Tahoe-X1\cite{gandhi2025tahoe} (12 layers, 1024 tokens, $\sim$70M parameters, pretrained on $\sim$271M cells), and scGPT\cite{cui2023scgpt} (12 layers, 5000 tokens, $\sim$100M parameters, pretrained on $\sim$33M cells) were all trained with massive compute budgets and far more pretraining data than is available for most biological problems. Even larger models in this space, such as TranscriptFormer\cite{pearce2025transcriptformer} (12 layers, 2047 tokens, 368--542M parameters depending on variant, pretrained on 57--112M cells across multiple species) or STATE\cite{adduri2025state} (8 layers, 512 tokens, $\sim$600M parameters for the embedding model, pretrained on large-scale perturbation datasets), were trained on even more extensive datasets spanning multiple species and hundreds of millions of cells, making such large-scale pretraining infeasible for systematic benchmarking. For our scaling law study, where model throughput matters and parameter counts must fit diverse data size regimes, using a reduced model is critical to feasibility.
...
\begin{table}[!htbp]
\centering
\caption{Architectural comparison of our Geneformer implementation with published single-cell transformer models.}
\label{tab:hyperparameter_comparison}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & \textbf{Layers} & \textbf{Hidden Dim.} & \textbf{Heads} & \textbf{Intermed. Size} & \textbf{Max Seq. Length} \\
\midrule
Our Geneformer & 3 & 256 & 4 & 512 & 512 \\
Geneformer-V1\cite{theodoris2023geneformer} & 6 & 256 & 4 & 512 & 2048 \\
Geneformer-V2-104M\cite{chen2024geneformer} & 12 & 768 & 12 & 3072 & 4096 \\
Tahoe-X1 (70M)\cite{gandhi2025tahoe} & 12 & 512 & 8 & 2048 & 1024 \\
scGPT\cite{cui2023scgpt} & 12 & 512 & 8 & 512 & 5000 \\
TranscriptFormer\cite{pearce2025transcriptformer} & 12 & 2048 & 16 & 2048 & 2047 \\
STATE (SE-600M)\cite{adduri2025state} & 8 & 768 & 12 & 3072 & 512 \\
\bottomrule
\end{tabular}
\end{table}
