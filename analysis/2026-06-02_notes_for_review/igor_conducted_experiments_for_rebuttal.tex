...
These 400 scalar values were plotted on a log--log scale.
...
\caption{Geneformer final test loss vs.\ training-set size on a
log--log scale, one curve per quality level.}
...
%% ============================================================
\section{KSG Mutual Information Estimates}
...
of quality with one curve per dataset size.
...
The probe type and plotted metric differ by dataset:

\begin{itemize}
  \item \textbf{PBMC} --- Ridge regression ($\alpha{=}1.0$) predicting
    surface \texttt{protein\_counts} (multi-output); plotted metric:
    mean $R^2$ across protein channels.
  \item \textbf{MERFISH} --- Ridge regression ($\alpha{=}1.0$) predicting
    ...
    space (\texttt{ng\_idx}); plotted metric: mean $R^2$.
  \item \textbf{larry} --- Lineage prediction via Ridge regression
    ...
    Plotted metric: mean $R^2$ across embedding dimensions.
  \item \textbf{shendure} --- Multi-class \texttt{author\_day}
    classification using a multinomial logistic regression
    (L-BFGS solver, max iterations 2\,000);
    plotted metric: accuracy.
\end{itemize}
...
and assembled into a single long-format table.
