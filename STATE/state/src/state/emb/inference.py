...
class Inference:
    ...
    def load_model(self, checkpoint):
        ...
        # Extract packaged protein embeddings from checkpoint
        if self.protein_embeds is None and isinstance(ckpt, dict) and "protein_embeds_dict" in ckpt:
        ...
        with torch.no_grad():
            ...
            with torch.autocast(device_type=device_type, dtype=precision):
                for i, batch in enumerate(dataloader):
                    ...
                    if hasattr(self.model, 'dataset_embedder') and self.model.dataset_embedder is not None:
